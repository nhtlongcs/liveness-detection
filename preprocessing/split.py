...
def sanity_check(df_raw, df_train, df_test):
    ...
    assert set(df_test["filename"].unique()).intersection(set(df_train["filename"].unique())) == set(), "Train and test sets should not have any common videos"

train_df, test_df = split(labels_df)
sanity_check(labels_df, train_df, test_df)
print(f"Train set size: {len(train_df)}, video count: {len(train_df['filename'].unique())}")
print(f"Test set size: {len(test_df)}, video count: {len(test_df['filename'].unique())}")

train_df.to_csv(f"{str(labels_path.parent)}/labels_video_train.csv", index=False)
