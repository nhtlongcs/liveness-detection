...
def train(config):
    ...
    trainer = pl.Trainer(
        ...
        strategy="ddp" if (torch.cuda.device_count() > 1 and not(config['global']['find_lr'])) else None,
        sync_batchnorm=True if (torch.cuda.device_count() > 1 and not(config['global']['find_lr'])) else False,
    ...
    if config['global']['find_lr']:
        ...
        table = tabulate.tabulate([['learning_rate', model.learning_rate]], headers=['name', 'value'], tablefmt='fancy_grid')
        print(table)
    else: 
        print("If this is the first time you run this model, you can use global.find_lr=True to find the best lr")
